...
inline std::string setbgcolor(uint8_t R, uint8_t G, uint8_t B) {
	std::stringstream ss;
	ss << "\033[48;2;" << (int)R << ";" << (int)G << ";" << (int)B << "m";
	return ss.str();
}
...
inline std::string setcolor(uint8_t R, uint8_t G, uint8_t B) {
	std::stringstream ss;
...
inline std::string movecursor(uint32_t row, uint32_t col) {
	std::stringstream ss;
	ss << "\033[" << row << ";" << col << "H";
	return ss.str();
}
...
inline std::string show_cursor(bool flag) {
	if (flag) {
		std::stringstream ss;
		ss << "\033[?25h";
		return ss.str();
...
inline std::string set_mouse_mode(bool flag) {
	std::stringstream ss;
